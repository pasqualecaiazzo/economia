const { graphConfigs, moduleCompanions, studyModules } = window.APP_DATA;

const state = {
  search: "",
  tag: "all",
  activeModuleId: studyModules[0].id,
  activeTab: "study",
...
const tags = ["all", "base", "grafico", "chiuse", "aperte", "micro", "macro", "digitale"];
...
function renderTagFilters() {
  els.tagFilters.innerHTML = tags
    .map(
      (tag) => `
        <button class="tag-pill ${state.tag === tag ? "active" : ""}" data-tag="${tag}">
          ${tag}
        </button>`
    )
    .join("");

  els.tagFilters.querySelectorAll(".tag-pill").forEach((button) => {
...
function getFilteredModules() {
  return studyModules.filter((module) => {
    const tagMatch =
      state.tag === "all" ||
      module.tags.includes(state.tag) ||
      module.category === state.tag;
    ...
    const searchMatch = !state.search || searchBucket.includes(state.search);
...
function renderModuleList() {
  ...
  if (!filtered.some((module) => module.id === state.activeModuleId)) {
    state.activeModuleId = filtered[0]?.id ?? studyModules[0].id;
  }

  els.moduleList.innerHTML = filtered
    .map(
      (module) => `
        <button class="module-button ${module.id === state.activeModuleId ? "active" : ""}" data-module-id="${module.id}">
          <span class="module-title">${module.title}</span>
          <div class="module-meta">
            <span class="mini-tag">${module.category}</span>
            ${module.tags.map((tag) => `<span class="mini-tag">${tag}</span>`).join("")}
          </div>
        </button>
      `
    )
...
function renderStudyCard() {
  const module = studyModules.find((item) => item.id === state.activeModuleId) ?? studyModules[0];

  els.studyCard.innerHTML = `
    <article class="study-card">
      <p class="eyebrow">${module.category} · modulo ripulito</p>
      <h2>${module.title}</h2>
      <p class="study-summary">${module.summary}</p>
      <div class="study-grid">
        ...
        <section class="study-section">
          <h3>Da ricordare a voce</h3>
          <ul>${module.remember.map((item) => `<li>${item}</li>`).join("")}</ul>
        </section>
        <section class="study-section">
          <h3>Errori frequenti</h3>
          <ul>${module.pitfalls.map((item) => `<li>${item}</li>`).join("")}</ul>
        </section>
        <section class="study-section">
          <h3>Lettura del grafico</h3>
          <p>${module.graphHint}</p>
        </section>
      </div>
      <div class="study-badges">
        ${module.tags.map((tag) => `<span class="study-badge">${tag}</span>`).join("")}
      </div>
    </article>
  `;
}
...
function renderFocusPanel() {
  const module = studyModules.find((item) => item.id === state.activeModuleId) ?? studyModules[0];
  const companion = moduleCompanions[module.id];

  els.focusMount.innerHTML = `
    <section class="focus-card">
      <p class="eyebrow">Ripasso attivo sul modulo selezionato</p>
      <h2>Interrogami: ${module.title}</h2>
      <p class="study-summary">${companion.cleanedNarrative}</p>
      <div class="study-grid">
        <section class="study-section">
        ...
        <section class="study-section">
          <h3>Rumore da ignorare</h3>
          <ul>${companion.noiseCuts.map((item) => `<li>${item}</li>`).join("")}</ul>
        </section>
        <section class="study-section">
          <h3>Domanda da orale</h3>
          <p>${companion.oralPrompt}</p>
          <textarea data-module-draft="${module.id}" placeholder="Prova a rispondere qui come se fossi all'orale...">${state.focusDrafts[module.id] ?? ""}</textarea>
        </section>
        <section class="study-section">
          <h3>Checklist finale</h3>
          <ul>${companion.oralChecks.map((item) => `<li>${item}</li>`).join("")}</ul>
        </section>
      </div>
    </section>
  `;

  els.focusMount.querySelector("textarea")?.addEventListener("input", (event) => {
    state.focusDrafts[event.target.dataset.moduleDraft] = event.target.value;
  });
}
...
function renderGraphControls() {
  ...
  graph.controls.forEach((control) => {
    ...
    el.addEventListener("input", (event) => {
      const nextValue = control.type === "range" ? Number(event.target.value) : event.target.value;
      state.graphState[state.graphId][control.id] = nextValue;
      renderGraphControls();
      drawGraph();
    });
  });
...
function drawMarketGraph(area, graphState) {
  ...
  if (policyLine !== null) {
    ...
    ctx.fillStyle = graphState.scenario === "price-ceiling" ? "rgba(207, 95, 53, 0.16)" : "rgba(30, 114, 137, 0.16)";
    ctx.fillRect(leftX, y - 10, rightX - leftX, 20);
    label(graphState.scenario === "price-ceiling" ? "Penuria" : "Eccedenza", (leftX + rightX) / 2 - 26, y - 16, "#1d2c31");
    ctx.restore();
  }
}
...
function drawIsoquantGraph(area, graphState) {
  ...
  drawCurve(area, iso, "#cf5f35", "Isoquanto", { x: Math.min(18, maxInput * 0.6), y: iso(Math.min(18, maxInput * 0.6)) }, [1.5, maxInput], [0, maxInput]);
  drawCurve(area, isocost, "#1e7289", "Isocosto", { x: Math.min(20, maxInput * 0.68), y: isocost(Math.min(20, maxInput * 0.68)) }, [0, maxInput], [0, maxInput]);
...
function drawCompetitionGraph(area, graphState) {
  ...
  if (qStar > 0 && qStar < maxQ) {
    ...
    ctx.fillStyle = price >= avc(qStar)
      ? (price >= atc(qStar) ? "rgba(145, 171, 107, 0.18)" : "rgba(207, 95, 53, 0.12)")
      : "rgba(29, 44, 49, 0.08)";
    ctx.fillRect(area.left, Math.min(yPrice, yAtc), xQ - area.left, Math.abs(yAtc - yPrice));
    label(price < avc(qStar) ? "Sospensione" : price >= atc(qStar) ? "Profitto" : "Perdita", area.left + (xQ - area.left) * 0.35, Math.min(yPrice, yAtc) - 10, "#1d2c31");
  }
}
...
function guidePoint(x1, y1, x2, y2, text, side) {
  ...
  if (side === "left-static") label(text, x2 + 8, y2 - 10);
  ctx.restore();
}
